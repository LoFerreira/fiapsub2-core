...
FIREBASE_JSON_LOCAL="fiapsub2-firebase-sdk.json"
#!/usr/bin/env bash
set -euo pipefail
...
SERVICE_FILE="k8s/service.yaml"
FIREBASE_SECRET_NAME="firebase-adminsdk"
FIREBASE_JSON_LOCAL="${ROOT_DIR}/fiapsub2-firebase-sdk.json"
FIREBASE_JSON_KEY="fiapsub2-firebase-sdk.json"
...
[[ -f "${FIREBASE_JSON_LOCAL}" ]] || { err "${FIREBASE_JSON_LOCAL} não encontrado"; exit 1; }
...
# 6) Secret do Firebase (idempotente via apply)
bold "🔐 Aplicando Secret ${FIREBASE_SECRET_NAME}"
...
kubectl rollout status deployment/fiapsub2-deployment -n "${NAMESPACE}" --timeout=180s || true
